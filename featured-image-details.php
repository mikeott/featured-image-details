...
function add_featured_image_display_settings( $content, $post_id ) {
    ...
    $fid_image_path         = get_the_post_thumbnail_url(get_the_ID(),'full');
    $fid_image_preview      = plugins_url( 'preview.php', __FILE__ ) . '?path=' . $fid_image_path . '&postID=' . $post_id;
    $fid_button             = sprintf('<p><a class="button fid-button">%1$s</a></p>',$button_label);
    $fid_script             = '<script>jQuery(".fid-button").click(function() { jQuery(document).ready(function(){jQuery(".fid-window").load("'. $fid_image_preview . '");}); jQuery(".fid-mask, .fid-window").fadeIn(); });</script>';
